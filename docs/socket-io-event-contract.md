...
## Connection
- Namespace/URL: not confirmed. Flutter reads `SOCKET_URL` from `--dart-define=SOCKET_URL=...` and does not connect until that value is set.
- Auth: not confirmed — token in handshake vs query param.
...
- Not confirmed — Flutter currently listens only. Confirm whether the client must emit heartbeats, ack, or SOS events.
...
- [ ] Confirm camelCase vs snake_case for REST payloads (current models mix both)
